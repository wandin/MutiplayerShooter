...
ASTrackerBot::ASTrackerBot()
{
	...
	HealthComp = CreateDefaultSubobject<USHealthComponent>(TEXT("HeatlComp"));
...
void ASTrackerBot::BeginPlay()
{
	...
	if (HasAuthority())
	{
		...
		//Every second we update our power-level based on nearbby bots
...
FVector ASTrackerBot::GetNextPathPoint()
{
	...
	// failed to find a path to player
...
void ASTrackerBot::OnCheckNearbyBots()
{
	...
	for (FOverlapResult Result : Overlaps)
	{
		...
		// Ignore this instance of trackerbot
		if (Bot && Bot != this)
		{
			NrOfBots++;
		}
	...
	if (MatInst)
	{
		// convert to a float between 0 and 1 just like an 'alpha' value of a texture. Now the material can be set up without having to know the max power less
		// which can be tweaed many times by gameplay decisions (would mean we need to keep 2places up to date)
		float Alpha = PowerLevel / (float)MaxPowerLevel; // casting 2 integers to a float value
...
void ASTrackerBot::Tick(float DeltaTime)
{
	...
	if (HasAuthority() && !bExploded)
	{
		...
		{
			//Keep moving towards next target (next path point)
